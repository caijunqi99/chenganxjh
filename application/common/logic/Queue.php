...
class Queue extends Model
{
    ...
    public function addExppoint($member_info)
    {
        ...
        $exppoints_model->saveExppointsLog('login', array(
            'exp_memberid' => $member_info['member_id'], 'exp_membername' => $member_info['member_name']
        ), true);
        return ds_callback(true);
    }
    ...
    {
        $update = \model('goodsfcode')->editGoodsFCode(array('fc_state' => 1), array('fc_id' => $fc_id));
        if (!$update) {
            return ds_callback(false, '更新F码使用状态失败fc_id:' . $fc_id);
        }
        else {
            return ds_callback(true);
        }
    ...
    public function createGoodsQRCode($param)
    {
        ...
        foreach ($param['goodsid_array'] as $goods_id) {
            // 生成商品二维码
            $PhpQRCode->set('date', url('goods/index', array('goods_id' => $goods_id)));
    ...
    public function clearSpecialGoodsPromotion($param)
    {
        ...
        Model('goods')->editGoods(array(
                                      'goods_promotion_price' => array('exp', 'goods_price'),
    ...
    public function sendVrCode($param)
    {
        ...
        $code_list = Model('vrorder')->getOrderCodeList($condition, 'vr_code,vr_indate');
        if (empty($code_list))
            return ds_callback(true);
        ...
        $tpl_info = Model('mailtemplates')->getTplInfo(array('code' => 'send_vr_code'));
        $data = array();
    ...
    public function build_pwdRedpacket($t_id)
    {
        ...
        if ($t_total <= 0) {
            return ds_callback(false, '红包模板信息错误');
        }
        while ($t_total > 0) {
            $is_succ = false;
            $insert_arr = array();
