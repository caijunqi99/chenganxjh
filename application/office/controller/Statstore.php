...
class Statstore extends AdminControl
{
    ...
    public function newstore(){
        ...
        if($search_type == 'day'){
            ...
            $where['store_add_time'] = array('between',array($stime,$etime));
            $field .= ' ,DAY(FROM_UNIXTIME(store_add_time)) as dayval,HOUR(FROM_UNIXTIME(store_add_time)) as hourval ';
            $memberlist = $model->getNewStoreStatList($where, $field, 0, '', 0);
            if($memberlist){
        ...
        if($search_type == 'week'){
            ...
            $where['store_add_time'] = array('between', array($stime,$etime));
            $field .= ',WEEKOFYEAR(FROM_UNIXTIME(store_add_time)) as weekval,WEEKDAY(FROM_UNIXTIME(store_add_time))+1 as dayofweekval ';
                ...
                if ($i <= $datanum){
                    ...
                    switch ($_GET['type']){
                        case 'ordernum':
                            ...
                            break;
                        case 'orderamount':
                            $tmp['orderamount'] = dsPriceFormat($store_arr[$k]['orderamount']);
                            break;
                    }
    ...
    public function showstore(){
        ...
        if (in_array(input('type'),array('newbyday','newbyweek','newbymonth','storearea'))){
            ...
            $where['store_add_time'] = array('between',$searchtime_arr);
        }
        //商品分类
        $sc_id = intval(input('scid'));
        if ($sc_id > 0){
            $where['sc_id'] = $sc_id;
    ...
    public function storesales_trends(){
        ...
        //查询数据
        $statlist_tmp = $model->statByStatorder($where, $field, 0, '');
        //整理统计数组
        $storename = '';
        if($statlist_tmp){
    ...
    public function storearea(){
        ...
        if (trim(input('param.search_time'))){
            $where['store_time'] = array('elt',strtotime(input('param.search_time')));
        }
        $search_sclass = intval(input('param.search_sclass'));
        if ($search_sclass > 0){
            $where['sc_id'] = $search_sclass;
        ...
        $statlist_tmp = $model->statByStore($where, $field, 0, 0, 'region_id', 'region_id');
        // 地区
        ...
        foreach ((array)$statlist_tmp as $k=>$v){
            ...
            $province_id = isset($v['province_id'])?intval($v['province_id']):'';
            $v['sort'] = $k+1;
