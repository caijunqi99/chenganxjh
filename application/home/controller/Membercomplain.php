    ...
    */
    ...
    public function complain_new()
    {
        ...
        if (isset($order_list[$order_id]['extend_complain'][$goods_id])&&intval($order_list[$order_id]['extend_complain'][$goods_id]) == 1) {//退款投诉
            $complain_subject = db('complainsubject')->where(array('complain_subject_id' => 1))->select();//投诉主题
    ...
    public function complain_save()
    {
        ...
        if ($if_complain < 1) {
            showDialog(lang('para_error'), url('memberorder/index'), 'error');
        }
        //检查是不是正在进行投诉
        if ($this->check_complain_exist($input['order_goods_id'])) {
            showDialog(lang('complain_repeat'), '', 'error');
        }
        ...
        if ($state) {
            showDialog(lang('complain_submit_success'), url('Membercomplain/index'), 'succ');
        } else {
    ...
    public function complain_cancel()
    {
        ...
        if (intval($complain_info['complain_state']) === 10) {
            ...
            showDialog(lang('complain_cancel_success'), 'reload', 'succ');
        } else {
            showDialog(lang('complain_cancel_fail'), '', 'error');
        }
    }
    ...
    private function upload_pic()
    {
        ...
        foreach ($complain_pic as $pic) {
            if (!empty($_FILES[$pic]['name'])) {
                ...
                $result=$upload->rule('uniqid')->validate(['ext'=>'jpg,jpeg,gif,png'])->move($uploaddir);
                if ($result) {
