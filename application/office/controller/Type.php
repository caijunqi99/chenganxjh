...
class Type extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'office/lang/zh-cn/type.lang.php');
        //获取当前角色对当前子目录的权限
    ...
    protected function getAdminItemList() {
        ...
        if(session('office_is_super') ==1 || in_array('3',$this->action)){
            if (request()->action() == 'type_edit') {
                $menu_array[] = array(
                    'name' => 'type_edit',
                    'text' => '编辑类型',
                    'url' => url('office/Member/type_edit')
                );
            }
        }
        ...
        if (request()->action() == 'attr_edit') {
            $menu_array[] = array(
                'name' => 'type_edit',
                'text' => '编辑属性',
                'url' => url('office/Member/attr_edit')
            );
        }
        return $menu_array;
    }
