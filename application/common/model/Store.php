...
class Store extends Model {
    ...
    public function getOwnShopIds($bind_all_gc = false) {
        ...
        if (!$data) {
            ...
            if (!$data) {
                $data = array();
                $all_own_shops = db('store')->field('store_id,bind_all_gc')->where(array(
                            'is_own_shop' => 1,
                        ))->select();
...
//        $result = db('store')->field($field)->where($condition)->order($order)->limit($limit)->page($page)->select();
        if ($page) {
    ...
    public function getStoreMemberIDList($storeid_array, $field = 'store_id,member_id,store_domain') {
        ...
        $store_list = ds_changeArraykey($store_list, 'store_id');
        return $store_list;
    }
    ...
    public function addStore($param) {
        return db('store')->insertGetId($param);
    }
