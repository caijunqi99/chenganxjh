...
class Memberorder extends BaseMember {
    ...
    public function index() {
        ...
        $order_list = $model_order->getOrderList($condition, 5, '*', 'order_id desc','', array('order_common','order_goods','store'));
        ...
        $order_pay_list = db('orderpay')->where('pay_sn', 'in', array_unique($order_pay_sn_array))->select();

        //更换数组的键值
        $order_pay_list = ds_changeArraykey($order_pay_list, 'pay_sn');
    ...
    function getMemberItemList() {
        $item_list = array(
            ...
            array(
                ...
                'text' => lang('回收站'),
                'url' => url('Home/Memberorder/index',['recycle'=>'1']),
            ),
        );
