...
class Order extends Model
{
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '')
    {
        try {
            ...
            $model_orderinviter->giveMoney($order_id);
            //添加会员积分
            ...
            if(!empty($inviter_id)) {
                $inviter_name = $model_member->getfby_member_id($inviter_id['inviter_id'], 'member_name');
                $rebate_amount = ceil(0.01 * $order_info['order_amount'] * config('points_rebate'));
                Model('points')->savePointsLog('rebate', array(
                    'pl_memberid' => $inviter_id['inviter_id'], 'pl_membername' => $inviter_name['member_name'], 'pl_points' => $rebate_amount
                ), true);
            }
