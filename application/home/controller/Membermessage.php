...
class Membermessage extends BaseMember
{
    public function _initialize()
    {
        ...
        Lang::load(APP_PATH . 'home/lang/zh-cn/membermessage.lang.php');
    }
    ...
    public function message()
    {
        ...
        $message_array = $model_message->listMessage(array('message_type' => '2', 'to_member_id_common' => session('member_id'), 'no_message_state' => '2'), 10);
    ...
    {
        $model_message = Model('message');
        $message_array = $model_message->listMessage(array('message_type' => '0', 'to_member_id_common' => session('member_id'), 'no_message_state' => '2'), 10);
        $this->assign('show_page',$model_message->page_info->render());
        $this->assign('message_array', $message_array);
    ...
    {
        $model_message = Model('message');
        $message_array = $model_message->listMessage(array('message_type_in' => '0,2', 'from_member_id' => session('member_id'), 'no_message_state' => '1'), 10);
        $this->assign('show_page',$model_message->page_info->render());
        $this->assign('message_array', $message_array);
    ...
    public function systemmsg()
    {
        ...
        $message_array = $model_message->listMessage(array('from_member_id' => '0', 'message_type' => '1', 'to_member_id' => session('member_id'), 'no_del_member_id' => session('member_id')), 10);
    ...
    public function sendmsg()
    {
        ...
        $friend_list = $friend_model->listFriend(array('friend_frommid' => session('member_id')));
    ...
    public function showmsgcommon()
    {
        ...
        $countnum = $model_message->countNewMessage(session('member_id'));
    ...
    public function dropbatchmsg()
    {
        ...
        if (!in_array($drop_type, array('msg_system', 'msg_seller')) || empty($message_id)) {
            showDialog(lang('home_message_delete_request_wrong'));
        }
        $messageid_arr = explode(',', $message_id);
        $messageid_str = '';
        if (!empty($messageid_arr)) {
        ...
        $drop_state = $model_message->dropBatchMessage($param, session('member_id'));
        if ($drop_state) {
            //更新未读站内信数量cookie值
            $cookie_name = 'msgnewnum' . session('member_id');
            $countnum = $model_message->countNewMessage(session('member_id'));
    ...
    public function setting()
    {
        ...
        $setting_list = $model_membermsgsetting->getMemberMsgSettingList(array('member_id' => session('member_id')));
    ...
    {
        $model_message = Model('message');
        $countnum = $model_message->countMessage(array('message_type' => '2', 'to_member_id_common' => session('member_id'), 'no_message_state' => '2', 'message_open_common' => '0'));
        return $countnum;
    }
    ...
    {
        $model_message = Model('message');
        $countnum = $model_message->countMessage(array('message_type' => '0', 'to_member_id_common' => session('member_id'), 'no_message_state' => '2', 'message_open_common' => '0'));
        return $countnum;
    }
    ...
    protected function getMemberItemList()
    {
        $menu_array = array(
            1 => array(
                ...
                'name' => 'setting', 'text' => '接收设置', 'url' => url('membermessage/setting')
            )
        );
        if (request()->action() == 'sendmsg') {
            $menu_array[] = array(
                'name' => 'sendmsg', 'text' => lang('home_message_send_message'),
                'url' => url('member_message/sendmsg')
            );
        }
