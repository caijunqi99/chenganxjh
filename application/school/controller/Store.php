...
class Store extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'school/lang/zh-cn/store.lang.php');
    }
...
//                $update_array['store_recommend'] = intval($_POST['store_recommend']);
            ...
            if ($result) {
                ...
                $this->success(lang('ds_common_save_succ'), url('School/Store/store'));
            } else {
                $this->log(lang('ds_edit').lang('store') . '[' . $_POST['store_name'] . ']', 1);
...
//                $store_update['area_info'] = $param['company_address'];
                ...
                $this->success(lang('ds_common_op_succ'), url('School/Store/store'));
            } else {
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        if ($param['paying_amount'] > 0) {
            $this->success('店铺入驻申请审核完成', url('School/Store/store_joinin'));
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if (input('post.verify_type') === 'pass') {
            ...
            if ($state) {
                ...
                $this->success('店铺开店成功', url('School/Store/store_joinin'));
            } else {
                $this->error('店铺开店失败', url('School/Store/store_joinin'));
            }
        } else {
            $this->error('店铺开店拒绝', url('School/Store/store_joinin'));
        }
    }
    ...
    public function newshop_add() {
        if (!request()->isPost()) {
            ...
            $this->success('操作成功', url('School/Store/store'));
            return;
        }
    }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'store',
                'text' => '管理',
                'url' => url('School/Store/store')
            ), array(
                'name' => 'store_joinin',
                'text' => '待审核',
                'url' => url('School/Store/store_joinin')
            ), array(
                'name' => 'reopen_list',
                'text' => '续签申请',
                'url' => url('School/Store/reopen_list')
            ), array(
                'name' => 'store_bind_class_applay_list',
                'text' => '经营类目申请',
                'url' => url('School/Store/store_bind_class_applay_list')
            ), array(
                'name' => 'newshop_add',
                'text' => '新增店铺',
                'url' => url('School/Store/newshop_add')
            )
        );
