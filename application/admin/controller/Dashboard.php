...
/*        //当日新增
    ...
    public function info(){
        ...
        if(!empty($_POST)){
            ...
            $result = db('admin')->where(array('admin_id'=>$admin_id))->field('admin_phone')->find();
            if($result['admin_phone'] == $admin_phone){
                $this->success('保存成功',url('Admin/Dashboard/info'));
            }
            $res = db('admin')->where(array('admin_id'=>$admin_id))->update(array('admin_phone'=>$admin_phone));
            if($res){
                $this->success('修改成功',url('Admin/Dashboard/info'));
            }else{
                $this->error('修改失败');
            }
        }else{
            $info = db('admin')
                ...
                ->where(array('a.admin_id'=>$admin_id))
                ->find();
            if($info){
                if($info['admin_is_super'] == 1){
                    $info['gname'] = '超级管理员';
                }
            }
            //        halt($info);
