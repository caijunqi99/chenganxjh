...
class Goodsbrowse extends Model {
    ...
    public function getViewedGoodsList($member_id = 0, $shownum = 0) {
        ...
        if ($member_id > 0) {
            if (!config('cache_open')) {//查询数据库
                $browselist_tmp = db('goodsbrowse')->where(array('member_id' => $member_id))->order('browsetime desc')->limit($shownum)->select();
    ...
    public function getGoodsbrowseOne($where, $field = '*', $order = '', $group = '') {
        db('goodsbrowse')->field($field)->where($where)->order($order)->group($group)->find();
    }
    ...
    public function getGoodsbrowseList($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '') {
        if ($page) {
            $result = db('goodsbrowse')->field($field)->where($where)->limit($limit)->order($order)->group($group)->paginate($page,false,['query' => request()->param()]);
