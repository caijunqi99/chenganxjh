...
class Groupbuyclass extends Model
{
    ...
    public function getList($condition = '', $page = '')
    {
        ...
        return db('groupbuyclass')->where($where)->order($order)->select();
    ...
    public function getOne($id)
    {
        if (intval($id) > 0) {
            $field = self::PK;
            $param['class_id'] = intval($id);
            $result = db('groupbuyclass')->field($field)->where($param)->find();
            return $result;
        }
        else {
            return false;
    ...
    {
        $where = $this->getCondition($condition);
        $list = db('groupbuyclass')->where($where)->select();
        if (empty($list)) {
            return false;
        }
        else {
    ...
    {
        return db('groupbuyclass')->insertGetId($param);
    ...
    public function updateinfo($update_array, $where_array)
    {
        ...
        return db('groupbuyclass')->where($where)->update($update_array);
    ...
    public function drop($param)
    {
        ...
        return db('groupbuyclass')->where($where)->delete();
    }
