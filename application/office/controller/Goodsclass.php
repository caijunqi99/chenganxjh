...
class Goodsclass extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'office/lang/zh-cn/goodsclass.lang.php');
        //获取当前角色对当前子目录的权限
    ...
    public function goods_class_add() {
        ...
        if (!request()->isPost()) {
            ...
            if ($result) {
                ...
                $this->success(lang('ds_common_save_succ'), url('office/Goodsclass/goods_class'));
            } else {
    ...
    public function goods_class_edit() {
        ...
        if (!request()->isPost()) {
            ...
            $this->success(lang('goods_class_batch_edit_ok'), url('office/Goodsclass/goods_class'));
        }
    }
    ...
    public function goods_class_del() {
        ...
        if ($gc_id > 0) {
            //删除分类
            $model_class->delGoodsClassByGcIdString($gc_id);
            $this->log(lang('ds_del,goods_class_index_class') . '[ID:' . $gc_id . ']', 1);
            $this->success(lang('ds_common_del_succ'), url('office/Goodsclass/goods_class'));
        } else {
            $this->log(lang('ds_del,goods_class_index_class') . '[ID:' . $gc_id . ']', 0);
            $this->error(lang('ds_common_del_fail'), url('office/Goodsclass/goods_class'));
        }
    }
    ...
    public function tag_reset() {
        ...
        if (!$return) {
            $this->error(lang('goods_class_reset_tag_fail'), url('office/Goodsclass/tag'));
        }
        ...
        if (is_array($goods_class) and !empty($goods_class)) {
            ...
            if ($return) {
                ...
                $this->success(lang('ds_common_op_succ'), url('office/Goodsclass/tag'));
            } else {
                $this->log(lang('ds_reset') . 'tag', 0);
                $this->error(lang('ds_common_op_fail'), url('office/Goodsclass/tag'));
            }
        } else {
            $this->log(lang('ds_reset') . 'tag', 0);
            $this->error(lang('goods_class_reset_tag_fail_no_class'), url('office/Goodsclass/tag'));
        }
    }
        ...
        if (is_array($tag_list) && !empty($tag_list)) {
            foreach ($tag_list as $val) {
                ...
                if (!$return) {
                    ...
                    $this->error(lang('ds_common_op_fail'), 'office/Goodsclass/tag');
                }
            }
            $this->log(lang('ds_update') . 'tag', 1);
            $this->success(lang('ds_common_op_succ'), 'office/Goodsclass/tag');
        } else {
            $this->log(lang('ds_update') . 'tag', 0);
            $this->error(lang('goods_class_update_tag_fail_no_class'), 'office/Goodsclass/tag');
        }
    }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'goods_class',
                'text' => '管理',
                'url' => url('office/Goodsclass/goods_class')
            ),
        );
        if(session('office_is_super') ==1 || in_array('1',$this->action)){
            $menu_array[] = array(
                ...
                'text' => '新增',
                'url' => url('office/Goodsclass/goods_class_add')
            );
        }
        ...
        if (request()->action() == 'goods_class_edit') {
            $menu_array[] = array(
                ...
                'text' => '编辑',
                'url' => url('office/Goodsclass/goods_class_edit')
            );
        }
        /*$menu_array[] = array(
            'name' => 'tag',
            'text' => '标签',
            'url' => url('office/Goodsclass/tag')
        );*/
        return $menu_array;
    }
