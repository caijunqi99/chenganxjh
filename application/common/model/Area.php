...
class Area extends Model
{
    ...
    public function getListArea($condition, $order = '')
    {
        ...
        $limit = $condition['limit'] ? $condition['limit'] : '';
        $c_order = $condition['order'] ? $condition['order'] : '';
        if (!empty($order)) {
            $c_order = $order;
        }
        $result =db('area')->field($field)->limit($limit)->order($c_order)->where($where)->select();
        return $result;
    }
    ...
    {
        return db('area')->where($condition)->field($fields)->group($group)->select();
    }
