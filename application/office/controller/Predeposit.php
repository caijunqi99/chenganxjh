...
class Predeposit extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'office/lang/zh-cn/predeposit.lang.php');
    }
    ...
    public function recharge_edit() {
        ...
        if ($id <= 0) {
            $this->error(lang('admin_predeposit_parameter_error'), 'office/Predeposit/pdrecharge_list');
        }
        //查询充值信息
        $model_pd = Model('predeposit');
        $condition = array();
        ...
        if (empty($info)) {
            $this->error(lang('admin_predeposit_record_error'), 'office/Predeposit/pdrecharge_list');
        }
        if (!request()->isPost()) {
            //显示支付接口列表
...
//            $model_pd->startTrans();
            ...
            $model_pd->commit();
            $this->log($log_msg, 1);
            $this->success(lang('admin_predeposit_recharge_edit_success'), 'office/Predeposit/pdrecharge_list');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg, 0);
            $this->error($e->getMessage(), 'office/Predeposit/pdrecharge_list');
        }
    }
    ...
    public function recharge_del() {
        ...
        if ($result) {
            $this->success(lang('admin_predeposit_recharge_del_success'), '/Office/Predeposit/pdrecharge_list');
        } else {
            $this->error(lang('admin_predeposit_recharge_del_fail'), '/Office/Predeposit/pdrecharge_list');
        }
    }
    ...
    public function pdcash_del() {
        ...
        if ($pdc_id <= 0) {
            $this->error(lang('admin_predeposit_parameter_error'), '/Office/Predeposit/pdcash_list');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        ...
        try {
            ...
            $model_pd->commit();
            $this->success(lang('admin_predeposit_cash_del_success'), '/Office/Predeposit/pdcash_list');
        } catch (Exception $e) {
            $model_pd->commit();
            $this->error($e->getMessage(), '/Office/Predeposit/pdcash_list');
        }
    }
    ...
    public function pdcash_pay() {
        ...
        if (!is_array($info) || count($info) < 0) {
            $this->error(lang('admin_predeposit_record_error'), '/Office/Predeposit/pdcash_list');
        }
        //查询用户信息
        $model_member = Model('member');
        $member_info = $model_member->getMemberInfo(array('member_id' => $info['pdc_member_id']));
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                ...
                'text' => '充值明细',
                'url' => url('office/Predeposit/pdrecharge_list')
            ),
            array(
                'name' => 'pdcash_list',
                'text' => '提现管理',
                'url' => url('office/Predeposit/pdcash_list')
            ),
            array(
                'name' => 'pdlog_list',
                'text' => '预存款明细',
                'url' => url('office/Predeposit/pdlog_list')
            ),
            array(
                'name' => 'pd_add',
                'text' => '预存款调节',
                'url' => url('office/Predeposit/pd_add')
            ),
        );
        return $menu_array;
    }
