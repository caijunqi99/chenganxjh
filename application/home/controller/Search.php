...
class Search extends BaseMall {
    ...
    public function index() {
        ...
        $this->assign('show_page', is_object($model_goods->page_info)?$model_goods->page_info->render():"");
        ...
        if ($keyword == '') {
            ...
            if (is_numeric($cate_id) && empty($keyword)) {
                $seo_info = $model_goods_class->getKeyWords($cate_id);
                if (empty($seo_info[1])) {
        ...
        // 当前位置导航
        $nav_link_list = $model_goods_class->getGoodsClassNav($cate_id);
        $this->assign('nav_link_list', $nav_link_list);

        // 得到自定义导航信息
        $nav_id = intval(input('param.nav_id')) ? intval(input('param.nav_id')) : 0;
