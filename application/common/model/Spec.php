...
class Spec extends Model {
    ...
    public function getSpecInfo($id, $field = '*') {
        return db('spec')->field($field)->find($id);
    }
    ...
    public function addSpecValue($insert) {
        $result = db('specvalue')->insert($insert);
        return $result;
    }
    ...
    public function addSpecValueALL($insert) {
        $result = db('specvalue')->insertAll($insert);
        return $result;
    }
    ...
    public function addSpec($param) {
        // 规格表插入数据
        $sp_id = db('spec')->insertGetId($param);
        return $sp_id;
    }
    ...
    public function specValueAdd($param) {
        return db('specvalue')->insertGetId($param);
    }
    ...
    public function specValueList($param, $page = '', $field = '*') {
        $condition_str = $this->getCondition($param);
        $list_spec = db('specvalue')->field($field)->where($condition_str)->select();
        return $list_spec;
    }
    ...
    public function delSpec($table, $param) {
        $condition_str = $this->getCondition($param);
        return db($table)->where($condition_str)->delete();
    }
