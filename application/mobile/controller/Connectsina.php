...
class Connectsina extends MobileMall
{
    ...
    public function register(){
        ...
        if (request()->isPost()){
            ...
            $this->success(lang('ds_common_save_succ'),SHOP_SITE_URL);
        }else{
            //检查登录状态
            //$model_member->checkloginMember();
            //获取新浪微博账号信息
            require_once (APP_PATH.'mobile'.DS.'api'.DS.'sina'.DS.'saetv2.ex.class.php');
    ...
    public function bindsina(){
        ...
        $sinauser_info = $c->show_user_by_id(session('slast_key.uid'));//根据ID获取用户等基本信息
        $sina_str = serialize($sinauser_info);
        $edit_state     = $model_member->editMember(array('member_id'=>session('member_id')),array('member_sinaopenid'=>session('slast_key.uid'), 'member_sinainfo'=>$sina_str));
        if ($edit_state){
            output_data('绑定QQ成功');
        }else {
