...
class Brand extends AdminControl
{
    ...
    {
        parent::_initialize();
        Lang::load(APP_PATH . 'office/lang/zh-cn/brand.lang.php');
    }
    ...
    protected function getAdminItemList()
    {
        $menu_array = array(
            array(
                'name' => 'index', 'text' => '管理', 'url' => url('office/Brand/index')
            ),
        );
        if (request()->action() == 'brand_add' || request()->action() == 'index') {
            $menu_array[] = array(
                'name' => 'brand_add', 'text' => '新增', 'url' => url('office/Brand/brand_add')
            );
        }
        if (request()->action() == 'brand_edit') {
            $menu_array[] = array(
                'name' => 'brand_edit', 'text' => '编辑', 'url' => url('office/Brand/brand_edit')
            );
        }
        return $menu_array;
    }
