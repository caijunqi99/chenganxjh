...
class Type extends Model{
    ...
    public function getGoodsAttrIndexList($conditoin, $page = 0, $fields = '', $order = '', $limit = '') {
        return db('goodsattrindex')->where($conditoin)->order($order)->limit($limit)->page($page)->select();
    }
    ...
    public function typeList($param, $page = '', $field = '*') {
        $list_type = db('type')->where($param)->select();
        return $list_type;
    }
    ...
    public function typeAdd($table, $param) {
        return db($table)->insertGetId($param);
    }
    ...
    public function typeRelatedList($table, $param, $field = '*') {
        $list_type = db($table)->field($field)->where($param)->select();
        return $list_type;
    }
    ...
    public function typeRelatedJoinList($param, $type = '', $field = '*', $order = '') {
        ...
        switch ($type) {
            ...
            case 'attr':
                ...
                $result = db('attributevalue')->alias('attribute_value')->join('__ATTRIBUTE__ attribute', 'attribute.attr_id=attribute_value.attr_id')->where($param)->order($order)->select();
                break;
            case 'brand':
                $param['brand_apply'] = 1;  //只查询通过的品牌
                $order = !empty($order) ? $order : 'brand.brand_initial asc, brand.brand_sort asc';
                $result = db('typebrand')->alias('type_brand')->join('__BRAND__ brand', 'type_brand.brand_id=brand.brand_id')->where($param)->order($order)->select();
                break;
        }
//        $result = db($table)->field($field)->join($join)->on($on)->where($param)->order($order)->select();
        return $result;
    }
