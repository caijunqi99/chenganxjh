...
//            $this->assign('gadmin', $gadmin);
            ...
            if ($rs) {
                ...
                $this->success(lang('ds_common_save_succ'), url('office/Office/admin'));
            } else {
    ...
    public function admin_edit() {
        ...
        if (request()->isPost()) {
            ...
            if ($result >=0) {
                ...
                $this->success(lang('admin_edit_success'), url('office/Office/admin'));
            } else {
                $this->error(lang('admin_edit_fail'), url('office/Office/admin'));
            }
        } else {
            //查询用户信息
            $admin_model = Model('admin');
            ...
            if (!is_array($admin) || count($admin) <= 0) {
                $this->error(lang('admin_edit_admin_error'), url('office/Office/admin'));
            }
            $company_id = db('admin')->field('admin_company_id')->where('admin_id = "'.session("admin_id").'"')->find();
        ...
        if(session('office_is_super') !=1 && !in_array('1',$this->action)){
            ...
            $menu_array = array(
                ...
                array(
                    'name' => 'admin_add',
                    'text' => '添加管理员',
                    'url' => url('office/Office/admin_add')
                )
            );
        }
