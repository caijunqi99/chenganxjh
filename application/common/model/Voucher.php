...
class Voucher extends Model
{
    ...
    public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0)
    {
        ...
        if(!empty($voucher_list)){
            $voucher_list = ds_changeArraykey($voucher_list);
        }
    ...
    public function getCurrentQuota($store_id)
    {
        ...
        $info = db('voucherquota')->where($param)->find();
        return $info;
    }
    ...
    public function getNewApply($store_id)
    {
        ...
        $new_apply = db('voucherapply')->where(array(
                                                   'apply_storeid' => $store_id,
    ...
    public function getVoucherList($where, $field = '*', $limit = 0, $page = 0, $order = '', $group = '')
    {
        ...
        if (is_array($page)) {
            if ($page[1] > 0) {
                $voucher_list = db('voucher')->field($field)->where($where)->limit($limit)->page($page[0], $page[1])->order($order)->group($group)->select();
            }
            else {
                $voucher_list = db('voucher')->field($field)->where($where)->limit($limit)->page($page[0])->order($order)->group($group)->select();
            }
        }
        else {
            $voucher_list = db('voucher')->field($field)->where($where)->order($order)->group($group)->select();
        }
        return $voucher_list;
    }
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null)
    {
       ...
       $list = db('voucher')->alias('v')->join('__STORE__ s','s.store_id=v.voucher_store_id')->join('__VOUCHERTEMPLATE__ t','v.voucher_t_id=t.voucher_t_id')->where($where)->order('voucher_id desc')->paginate($page,false,['query' => request()->param()]);
        $this->page_info=$list;
        $list=$list->items();
    ...
    public function getVoucherTemplateList($where, $field = '*', $limit = 0, $page = 0, $order = '', $group = '')
    {
        ...
        if (is_array($page)) {
            if ($page[1] > 0) {
                $voucher_list = db('vouchertemplate')->field($field)->where($where)->page($page[0], $page[1])->order($order)->group($group)->select();
            }
            else {
                $voucher_list = db('vouchertemplate')->field($field)->where($where)->page($page[0])->order($order)->group($group)->select();
            }
        }
        else {
            $voucher_list = db('vouchertemplate')->field($field)->where($where)->order($order)->group($group)->paginate(10,false,['query' => request()->param()]);
        }
        $this->page_info=$voucher_list;
        $voucher_list=$voucher_list->items();
        //查询店铺分类
    ...
    {
        return db('voucherprice')->order('voucher_price asc')->select();
    }
    ...
    public function exchangeVoucher($template_info, $member_id, $member_name = '')
    {
        ...
        $result = db('voucher')->insert($insert_arr);
        if (!$result) {
        ...
        if ($result) {
            //代金券模板的兑换数增加
            $result = $this->editVoucherTemplate(array('voucher_t_id' => $template_info['voucher_t_id']), array(
                'voucher_t_giveout' => array(
                    'exp', 'voucher_t_giveout+1'
                )
            ));
            if (!$result) {
                return array('state' => false, 'msg' => '兑换失败');
    ...
    {
        $info = db('vouchertemplate')->where($where)->field($field)->order($order)->group($group)->find();
        if (!$info) {
            return array();
        }
    ...
    {
        $info = db('voucher')->where($where)->field($field)->order($order)->group($group)->find();
