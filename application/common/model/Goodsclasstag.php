...
class Goodsclasstag extends Model {
    ...
    public function getTagList($condition = array(), $page = '', $field = '*') {
        if ($page) {
            $result = db('goodsclasstag')->where($condition)->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            $result = db('goodsclasstag')->where($condition)->select();
            return $result;
        }
    }
    ...
    public function tagAdd($param) {
        ...
        return Db::query("insert into `" . "ds_goodsclasstag` (`gc_id_1`,`gc_id_2`,`gc_id_3`,`gc_tag_name`,`gc_tag_value`,`gc_id`,`type_id`) values " . $condition_str);
    }
    ...
    public function clearTag() {
        return Db::query("TRUNCATE TABLE `" . 'ds_' . "goodsclasstag`");
    }
     ...
     * @return bool
     */
    public function updateTag($param,$gc_tag_id) {
        return db('goodsclasstag')->where("gc_tag_id = '" . $gc_tag_id . "'")->update($param);
    }
    ...
    public function delTagByIds($id) {
        return db('goodsclasstag')->where('goods_class_tag', 'gc_tag_id in (' . $id . ')')->delete();
    }
    ...
    public function addOneTag($param) {
        return db('goodsclasstag')->insertGetId($param);
    }
