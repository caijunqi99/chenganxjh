...
class Pointprod extends Model
{
    ...
    public function addPointGoods($param)
    {
        ...
        $insert_id = db('pointsgoods')->insert($param);
        if ($insert_id) {
            return $insert_id;
        }
        else {
    ...
    public function getPointProdList($where = '', $field = '*', $order = '', $limit = '', $page = '')
    {
        ...
        if ($limit) {
            $list = db('pointsgoods')->field($field)->where($where)->order($order)->limit($limit)->page($page)->select();
        }
        else {
        ...
        if ($result) {
            ...
            if (is_array($pg_id) && count($pg_id) > 0) {
                ...
                $upload_list = $upload_model->getUploadList(array(
                                                                'upload_type_in' => '5,6', 'item_id_in' => $pg_idStr
                                                            ));
            }
            else {
