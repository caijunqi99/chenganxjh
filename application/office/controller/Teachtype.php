...
class Teachtype extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'office/lang/zh-cn/teacher.lang.php');
        //获取当前角色对当前子目录的权限
    ...
    public function type_class_add() {
        ...
        if (!request()->isPost()) {
            ...
            if ($result) {
                ...
                $this->success(lang('ds_common_save_succs'), url('office/Teachtype/index'));
            } else {
    ...
    public function type_class_edit() {
        ...
        if (!request()->isPost()) {
            ...
            $this->success(lang('goods_class_batch_edit_ok'), url('office/Teachtype/index'));
        }
    }
    ...
    public function type_class_del() {
        ...
        if ($gc_id > 0) {
            ...
            $this->success(lang('ds_common_del_succ'), url('office/Teachtype/index'));
        } else {
            $this->log(lang('ds_del,goods_class_index_class') . '[ID:' . $gc_id . ']', 0);
            $this->error(lang('ds_common_del_fail'), url('office/Teachtype/index'));
        }
    }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'index',
                'text' => '管理',
                'url' => url('office/Teachtype/index')
            ),
        );
        if(session('office_is_super') ==1 || in_array(1,$this->action )){
            if (request()->action() == 'type_class_add' || request()->action() == 'index') {
                $menu_array[] = array(
                    'name' => 'type_class_add',
                    'text' => '新增',
                    'url' => url('office/Teachtype/type_class_add')
                );
            }
        }
        if (request()->action() == 'type_class_edit') {
            $menu_array[] = array(
                ...
                'text' => '编辑',
                'url' => url('office/Teachtype/type_class_edit')
            );
        }
        return $menu_array;
    }
