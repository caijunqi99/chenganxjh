...
class Buy extends Model
{
    ...
    public function addOrderInviter($order_list = array()) {
        if (empty($order_list) || !is_array($order_list))
            return;
        $inviter_ratios=db('config')->where('code','in', 'inviter_ratio_1,inviter_ratio_2,inviter_ratio_3')->column('value');
        $model_orderinviter = Model('orderinviter');
        foreach ($order_list as $order_id => $order) {
            ...
            if($inviter_1 && isset($inviter_ratios[0]) && floatval($inviter_ratios[0])>0){
                $money_1=round($inviter_ratios[0]/100*$order['goods_amount'],2);
                if($money_1>0){
                    try {
                        $model_orderinviter->startTrans();
                        //生成推广记录
                        ...
                        $model_orderinviter->commit();
                    } catch (Exception $e) {
                        $model_orderinviter->rollback();
                    }
                }
            }
            //二级推荐人
            ...
            if($inviter_2 && isset($inviter_ratios[1]) && floatval($inviter_ratios[1])>0){
                $money_2=round($inviter_ratios[1]/100*$order['goods_amount'],2);
                if($money_2>0){
                    try {
                        $model_orderinviter->startTrans();
                        //生成推广记录
            ...
            if($inviter_3 && isset($inviter_ratios[2]) && floatval($inviter_ratios[2])>0){
                $money_3=round($inviter_ratios[2]/100*$order['goods_amount'],2);
                if($money_3>0){
                    try {
                        $model_orderinviter->startTrans();
                        //生成推广记录
    ...
    {
        $res=model('goods')->getGoodsInfo(array('goods_id'=>$goods_goodid),'goods_commonid');
        $fcode_info = Model('goodsfcode')->getGoodsFCode(array(
                                                              'goods_commonid' => $res['goods_commonid'], 'fc_code' => $fcode,
                                                              'fc_state' => 0
                                                          ));
        if ($fcode_info) {
            ...
            foreach ($goods_list as $goods_info) {
                ...
                elseif (!empty($goods_info['bl_goods_list']) && is_array($goods_info['bl_goods_list'])) {
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        ...
                        $order_goods[$i]['commis_rate'] = isset($store_gc_id_commis_rate_list[$store_id])?floatval($store_gc_id_commis_rate_list[$store_id][$goods_info['gc_id']]):'0';
    ...
    private function _createOrderStep6()
    {
        ...
        //更新F码使用状态
        if ($fc_id) {
            \mall\queue\QueueClient::push('updateGoodsFCode', $fc_id);
        }
