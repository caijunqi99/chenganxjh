...
class Teachvideo extends AdminControl {
    ...
    public function add() {
        ...
        if (!request()->isPost()) {
            ...
            if ($result) {
                $this->success(lang('ds_common_save_succ'), url('office/Teachvideo/index'));
            } else {
                $this->error(lang('ds_common_save_fail'), url('office/Teachvideo/index'));
            }
    ...
    public function image($picture){
        ...
        {
            $this->error(lang("格式不正确"), url('office/Teachvideo/index'));
        }
        if (!empty($_FILES['video_filename']['name'])) {
            ...
            if($upload){
                ...
                $this->error(lang("上传视频封面图失败"), url('office/Teachvideo/index'));
            }
        }
    }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'index',
                'text' => '管理',
                'url' => url('office/Teachvideo/index')
            ),
        );
        if(session('office_is_super') ==1 || in_array(1,$this->action )){
        ...
        if (request()->action() == 'edit') {
            $menu_array[] = array(
                'name' => 'edit',
                'text' => '编辑',
                'url' => url('office/Teachvideo/edit')
            );
        }
        if (request()->action() == 'pass') {
            $menu_array[] = array(
                'name' => 'pass',
                'text' => '审核',
                'url' => url('office/Teachvideo/pass')
            );
        }
        return $menu_array;
    }
