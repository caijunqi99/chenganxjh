...
class Message extends AdminControl {
    ...
    public function _initialize() {
        ...
        Lang::load(APP_PATH . 'school/lang/zh-cn/message.lang.php');
    }
        ...
        if (!request()->isPost()) {
            ...
            if (!$error) {
                ...
                if ($result) {
                    ...
                    $this->success(lang('mailtemplates_edit_succ'), 'School/Message/email_tpl');
                } else {
    ...
    private function seller_tpl_update_showmessage($result) {
        if ($result) {
            $this->success(lang('ds_common_op_succ'), url('School/Message/seller_tpl'));
        } else {
    ...
    private function member_tpl_update_showmessage($result) {
        if ($result) {
            $this->success(lang('ds_common_op_succ'), url('School/message/member_tpl'));
        } else {
            $this->error(lang('ds_common_op_fail'));
        }
    }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'email',
                'text' => '邮件设置',
                'url' => url('School/Message/email')
            ),
            array(
                'name' => 'mobile',
                'text' => '短信平台设置',
                'url' => url('School/Message/mobile')
            ),
            array(
                'name' => 'seller_tpl',
                'text' => '商家消息模板',
                'url' => url('School/Message/seller_tpl')
            ),
            array(
                'name' => 'member_tpl',
                'text' => '用户消息模板',
                'url' => url('School/Message/member_tpl')
            ),
            array(
                'name' => 'email_tpl',
                'text' => '其他模板',
                'url' => url('School/Message/email_tpl')
            ),
        );
