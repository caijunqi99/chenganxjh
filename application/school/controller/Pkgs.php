        ...
        if (request()->isPost()) {            
            ...
            if (!$pkg_id) {//新增                                                
                ...
                if ($result) {
                    ...
                    $this->success(lang('pkg_add_succ'), url('School/Pkgs/pkgs_manage'),array('pkg_type'=>$param['pkg_type']));
                } else {
                ...
                if ($result) {
                    ...
                    $this->success(lang('pkg_edit_succ'), url('School/Pkgs/pkgs_manage'),array('pkg_type'=>$param['pkg_type']));
                } else {
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                ...
                'url' => url('School/Pkgs/pkgs_manage',['pkg_type'=>1])
            )
        );
        $menu_array[] = array(
             'name' => 'witch_back_manage',
                'text' => lang('witch_back_manage'),
                'url' => url('School/Pkgs/pkgs_manage',['pkg_type'=>2])
        );
        $menu_array[] = array(
            'name' => 'courseware',
            'text' => lang('courseware'),
            'url' => url('School/Pkgs/courseware',['pkg_type'=>3])
        );
        $menu_array[] = array(
            'name' => 'pkgs_edit',
            'text' => lang('pkgs_edit'),
            'url' => url('School/Pkgs/pkgs_edit')
        );
        return $menu_array;
    }
