...
class Selleractivity extends BaseSeller
{
    ...
    public function index()
    {
        ...
        //活动为商品活动，并且为开启状态
        $list = $activity->getList(array(
                                       'activity_type' => '1', 'opening' => true,
                                       'order' => 'a.activity_sort asc'
                                   ), 10);
        /**
         * 页面输出
         */
    ...
    public function activity_apply()
    {
        ...
        if ($activity_id <= 0) {
            $this->error(lang('para_error'), 'Selleractivity/index');
        }
       ...
       /* $gc = Model('goodsclass');
        ...
        //查询活动细节信息
        $activity_detail_model = Model('activitydetail');
        $list = $activity_detail_model->getGoodsJoinList(array(
    ...
    public function activity_apply_save()
    {
        ...
        if (empty($_POST['item_id'])) {
            showDialog(lang('store_activity_choose_goods'), url('Selleractivity/index'));
        }
        $activity_id = intval($_POST['activity_id']);
        if ($activity_id <= 0) {
            showDialog(lang('para_error'), url('Selleractivity/index'));
        }
        ...
        if (empty($activity) || $activity['activity_type'] != '1' || $activity['activity_state'] != '1' || $activity['activity_start_date'] > time() || $activity['activity_end_date'] < time()) {
            showDialog(lang('store_activity_not_exists'), url('Selleractivity/index'));
        }
        $activity_detail = Model('activitydetail');
        $list = $activity_detail->getList(array(
                                              'store_id' => "{session('store_id')}", 'activity_id' => "$activity_id"
                                          ));
        ...
        showDialog(lang('store_activity_submitted'), 'reload', 'succ');
    }
