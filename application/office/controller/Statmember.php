...
class Statmember extends AdminControl
{
    public function _initialize()
    {
        ...
        Lang::load(APP_PATH.'office/lang/zh-cn/stat.lang.php');
    ...
    public function newmember(){
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            $where['member_add_time'] = array('between',array($stime,$etime));
            $field .= ' ,DAY(FROM_UNIXTIME(member_add_time)) as dayval,HOUR(FROM_UNIXTIME(member_add_time)) as hourval ';
            $memberlist = $model->statByMember($where, $field, 0, '', '');
            if($memberlist){
        ...
        if($this->search_arr['search_type'] == 'week'){
            ...
            $where['member_add_time'] = array('between', array($stime,$etime));
            $field .= ',WEEKOFYEAR(FROM_UNIXTIME(member_add_time)) as weekval,WEEKDAY(FROM_UNIXTIME(member_add_time))+1 as dayofweekval ';
            $memberlist = $model->statByMember($where, $field, 0, '');
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            $where['member_add_time'] = array('between', array($stime,$etime));
            $field .= ',MONTH(FROM_UNIXTIME(member_add_time)) as monthval,day(FROM_UNIXTIME(member_add_time)) as dayval ';
            $memberlist = $model->statByMember($where, $field, 0, '');
            if($memberlist){
                foreach($memberlist as $k=>$v){
    ...
    public function analyzeinfo(){
        ...
        if (input('param.exporttype') == 'excel'){
            $memberlist = $model->statByStatmember($where, $field, 0, 0, "{input('param.type')} desc,statm_memberid desc", 'statm_memberid');
        } else {
            $memberlist = $model->statByStatmember($where, $field, array(10,$countnum), 0, "{input('param.type')} desc,statm_memberid desc", 'statm_memberid');
        }
    ...
    public function showmember(){
        ...
        if (isset($this->search_arr['exporttype'])&&$this->search_arr['exporttype'] == 'excel'){
            ...
            //data
            foreach ($member_list as $k=>$v){
                $excel_data[$k+1][] = array('data'=>$v['member_name'].'('.lang('member_index_true_name,ds_colon').$v['member_truename'].')');
                ...
                $excel_data[$k+1][] = array('data'=>lang('member_index_available,ds_colon').$v['available_predeposit'].lang('currency_zh').'('.lang('member_index_frozen,ds_colon').$v['freeze_predeposit'].lang('currency_zh').')');
            }
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
    ...
    public function buying(){
        ...
        $orderlist = $model->statByStatorder($where, $field, 0, 0, '', '');
        $stat_arr = array();
        $stat_arr['series'][0]['name'] = '下单量';
        //构造横轴坐标
        for ($i=0; $i<24; $i++){
