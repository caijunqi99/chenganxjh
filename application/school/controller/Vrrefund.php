...
class Vrrefund extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'school/lang/zh-cn/vrrefund.lang.php');
    ...
    public function edit() {
        ...
        if (!(request()->isPost())) {
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => url('School/Membervrrefund/view',['refund_id'=>$refund['refund_id']]),
                    'refund_sn' => $refund['refund_sn']
                );
                \mall\queue\QueueClient::push('sendMemberMsg', $param);

                $this->log('虚拟订单退款审核，退款编号' . $refund['refund_sn']);
                $this->success(lang('ds_common_save_succ'), url('School/vrrefund/refund_manage'));
            } else {
                $this->error(lang('ds_common_save_fail'));
            }
        }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'refund_manage',
                'text' => '待审核',
                'url' => url('School/Vrrefund/refund_manage')
            ),
            array(
                'name' => 'refund_all',
                'text' => '所有记录',
                'url' => url('School/Vrrefund/refund_all')
            ),
        );
        return $menu_array;
    }
