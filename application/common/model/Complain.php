...
class Complain extends Model {
    ...
    public function saveComplain($param) {
        ...
        return db('complain')->insert($param);
    }
    ...
    public function updateComplain($update_array, $where_array) {
        ...
        return db('complain')->where($where)->update($update_array);
    }
    ...
    public function dropComplain($param) {
        ...
        return db('complain')->where($where)->delete();
    }
    ...
    public function getComplain($condition = '', $page = '') {
        ...
        $param = $this->getCondition($condition);
        $order = $condition['order'] ? $condition['order'] : ' complain_id desc ';
        if ($page) {
            $res = db('complain')->where($param)->order($order)->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $res;
            return $res->items();
        } else {
            return db('complain')->where($param)->order($order)->select();
        }
    }
    ...
    public function getComplainGoodsList($complain_list) {
        ...
        return ds_changeArraykey($res, 'rec_id');
    }
    ...
    public function isExist($condition = '') {
        $param = $this->getCondition($condition);
        $list = db('complain')->where($param)->select();
        if (empty($list)) {
            return false;
        } else {
            return true;
    ...
    public function getoneComplain($complain_id) {
        $param = array();
        $param['complain_id'] = intval($complain_id);
        //return Db::getRow($param) ;
        return db('complain')->where($param)->find();
    }
    ...
    public function getCount($condition) {
        ...
        $count = db('complain')->where($condition_str)->count();
        return $count;
    }
