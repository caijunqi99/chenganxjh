...
class Returnmanage extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'school/lang/zh-cn/returnmanage.lang.php');
        //向模板页面输出退款退货状态
    ...
    public function edit() {
        ...
        if (request()->isPost()) {
            ...
            if ($state) {
                ...
                $this->success(lang('ds_common_save_succ'), 'returnmanage/return_manage');
            } else {
                $this->error(lang('ds_common_save_fail'));
            }
        }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'return_manage',
                'text' => '待审核',
                'url' => url('School/returnmanage/return_manage')
            ),
            array(
                'name' => 'return_all',
                'text' => '所有记录',
                'url' => url('School/returnmanage/return_all')
            ),
        );
        if(request()->action() == 'edit') {
            $menu_array[] = array(
                'name' => 'edit', 'text' => '审核', 'url' => '',
            );
        }
        return $menu_array;
    }
