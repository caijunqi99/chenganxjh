...
class Goods extends AdminControl
{
    ...
    {
        parent::_initialize();
        Lang::load(APP_PATH . 'office/lang/zh-cn/goods.lang.php');
        //获取当前角色对当前子目录的权限
    ...
    public function goods_lockup() {
        if(session('office_is_super') !=1 && !in_array('16',$this->action)){
            $this->error(lang('ds_assign_right'));
        }
        if (request()->isPost()) {
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'index',
                'text' => '所有商品',
                'url' => url('office/Goods/index')
            ),
            array(
                'name' => 'lockup',
                'text' => '下架商品',
                'url' => url('office/Goods/index',['type'=>'lockup'])
            ),
            array(
                'name' => 'waitverify',
                'text' => '待审核',
                'url' => url('office/Goods/index',['type'=>'waitverify'])
            ),
        );
        return $menu_array;
    }
