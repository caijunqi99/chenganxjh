...
class Teachtype extends Model
{
                    ...
                    foreach ((array)$data['children'][$id] as $cid) {
                        if (!empty($data['children'][$cid])) {
                            foreach ((array)$data['children'][$cid] as $ccid) {
                                ...
                                if(!empty($data['children2'][$cid])){
                                    foreach ((array)$data['children'][$cid] as $cccid) {
                                        $data['children3'][$id][] = $cccid;
                                    }
                                }
                            }
                        }
     ...
     */
    public function editTypeClass($data = array(), $condition = array())
    {
        // 删除缓存
        $this->dropCache();
    ...
    public function getGoodsClass($store_id, $pid = 0, $deep = 1)
    {
        ...
        if (!checkPlatformStoreBindingAllGoodsClass()) {
            ...
            $model_storebindclass = model('storebindclass');
     ...
     * @return boolean
     */
    public function delTypeClassByGcIdString($gcids)
    {
        $gcids = explode(',', $gcids);
        if (empty($gcids)) {
            return false;
        ...
        // 删除常用商品分类
        Model('goodsclassstaple')->delStaple(array('gc_id_1|gc_id_2|gc_id_3' => array('in', $gcid_array)));
        // 删除分类tag表
        ...
        // 商品下架
        Model('goods')->editProducesLockUp(array('goods_stateremark' => '商品分类被删除，需要重新选择分类'), array(
            'gc_id' => array(
                'in', $gcid_array
            )
        ));
            ...
            if (is_array($class_list) && !empty($class_list)) {
                ...
                foreach ($gc_list as $key => $value) {
                    ...
                    if (file_exists($pic_name)) {
                        $gc_list[$gc_id]['pic'] = BASE_UPLOAD_PATH . '/' . ATTACH_COMMON . '/category-pic-' . $gc_id . '.jpg';
                    }
    ...
    public function get_type_brands($type_ids = array())
    {
        ...
        if (is_array($type_ids) && !empty($type_ids)) {
            ...
            if (is_array($type_list) && !empty($type_list)) {
                $brand_list = db('brand')->field('brand_id,brand_name,brand_pic')->where(array('brand_apply' => 1))->limit(10000)->select();
    ...
    private function _getGoodsClassNavById($gc_id)
    {
        ...
        if ($nav_info['cn_brandids'] != '') {
            $nav_info['cn_brands'] = $model_brand->getBrandList(array(
                'brand_id' => array(
                    'in', $nav_info['cn_brandids']
                )
            ));
     ...
     * @return boolean
     */
    public function addTypeClass($insert)
    {
        // 删除缓存
        $this->dropCache();
     ...
     */
    public function getTreeTypeList($show_deep = '4', $condition = array())
    {
        ...
        if (is_array($class_list) && !empty($class_list)) {
            ...
            for ($i; $i < $size; $i++) {//$i为上次循环到的分类编号，避免重新从第一条开始
                ...
                if ($gc_parent_id == $parent_id) {
                    $val['deep'] = $deep;
                    $show_class[] = $val;
                    if ($deep < $show_deep && $deep < 4) {//本次深度小于显示深度时执行，避免取出的数据无用
                        //v3-b12
