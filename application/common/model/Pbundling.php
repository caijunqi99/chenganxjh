...
class Pbundling extends Model
{
    ...
    {
        if ($limit) {
            return db('pbundling')->where($condition)->order($order)->limit($limit)->page($page, $count)->select();
        }
        else {
    ...
    {
        return db('pbundling')->insertGetId($insert, $replace);
    }
    ...
    {
        if ($limit) {
            return db('pbundlingquota')->where($condition)->order('bl_quota_id desc')->limit($limit)->page($page)->select();
        }
        else {
            $res = db('pbundlingquota')->where($condition)->order('bl_quota_id desc')->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $res;
            return $res->items();
        }
    }
    ...
    {
        return db('pbundlingquota')->insert($insert);
    }
    ...
    {
        $result = db('pbundlinggoods')->insertAll($insert);
        if ($result) {
                    ...
                    if (!empty($b_goods_list) && count($b_goods_list) > 1) {
                        ...
                        foreach ($b_goods_list as $val) {
                            if (isset($goods_list[$val['goods_id']])) {
                                ...
                                $b_goods_array[$val['bl_id']][$k]['shop_price'] = dsPriceFormat($goods_list[$val['goods_id']]['goods_price']);
                                $b_goods_array[$val['bl_id']][$k]['price'] = dsPriceFormat($val['bl_goods_price']);
