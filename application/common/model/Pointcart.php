...
class Pointcart extends Model
{
    ...
    public function addPointCart($param)
    {
        ...
        $result = db('pointscart')->insert($param);
        if ($result) {
            //清除相关缓存
    ...
    public function getPointCartList($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '')
    {
        if (is_array($page)) {
            if ($page[1] > 0) {
                $cartgoods_list = db('pointscart')->field($field)->where($where)->page($page[0], $page[1])->limit($limit)->group($group)->order($order)->select();
            }
            else {
                $cartgoods_list = db('pointscart')->field($field)->where($where)->page($page[0])->limit($limit)->group($group)->order($order)->select();
            }
        }
        else {
            $cartgoods_list = db('pointscart')->field($field)->where($where)->page($page)->group($group)->order($order)->select();
        }
        if ($cartgoods_list) {
    ...
    {
        return db('pointscart')->field($field)->where($where)->group($group)->order($order)->find();
    }
    ...
    public function editPointCart($where, $param)
    {
        ...
        $result = db('pointscart')->where($where)->update($param);
        return $result;
    }
