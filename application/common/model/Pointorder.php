...
class Pointorder extends Model {
    ...
    public function point_snOrder($member_id) {
        return $this->product_sn = mt_rand(10, 99) . sprintf('%010d', time() - 946656000) . sprintf('%03d', (float) microtime() * 1000) . sprintf('%03d', (int) $member_id % 1000);
    }
    ...
    public function addPointOrder($param) {
        ...
        $result = db('pointsorder')->insertGetId($param);
        //清除相关缓存
    ...
    public function addPointOrderProdAll($param) {
        ...
        $result = db('pointsordergoods')->insertAll($param);
        return $result;
    }
    ...
    public function getPointOrderGoodsList($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '') {
        ...
        if (is_array($page)) {
            if ($page[1] > 0) {
                $ordergoods_list = db('pointsordergoods')->field($field)->where($where)->group($group)->page($page[0], $page[1])->limit($limit)->order($order)->select();
            } else {
                $ordergoods_list = db('pointsordergoods')->field($field)->where($where)->group($group)->page($page[0])->limit($limit)->order($order)->select();
            }
        } else {
            $ordergoods_list = db('pointsordergoods')->field($field)->where($where)->group($group)->page($page)->order($order)->select();
        }
        if ($ordergoods_list) {
    ...
    public function addPointOrderAddress($param) {
        ...
        return db('pointsorderaddress')->insert($param);
    }
    ...
    public function getPointOrderList($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '') {
        if (is_array($page)) {
            if ($page[1] > 0) {
                $order_list = db('pointsorder')->field($field)->where($where)->group($group)->page($page[0], $page[1])->limit($limit)->order($order)->paginate(10,false,['query' => request()->param()])->select();
            } else {
                $order_list = db('pointsorder')->field($field)->where($where)->group($group)->page($page[0])->limit($limit)->order($order)->select();
            }
        } else {
            $order_list = db('pointsorder')->field($field)->where($where)->group($group)->order($order)->paginate($page,false,['query' => request()->param()]);
    ...
    public function editPointOrder($where, $param) {
        ...
        return db('pointsorder')->where($where)->update($param);
    }
    ...
    public function cancelPointOrder($point_orderid, $member_id = 0) {
        ...
        if ($result) {
            ...
            //更改兑换礼品库存
            $prod_list = $this->getPointOrderAndGoodsList(array('pointsordergoods.point_orderid' => $order_info['point_orderid']), 'point_goodsid,point_goodsnum');
            if ($prod_list) {
                ...
                foreach ($prod_list as $v) {
                    ...
                    $update_arr['pgoods_storage'] = array(
                        'exp',
                        "pgoods_storage+{$v['point_goodsnum']}"
                    );
                    $update_arr['pgoods_salenum'] = array(
                        'exp',
                        "pgoods_salenum-{$v['point_goodsnum']}"
                    );
    ...
    public function getPointOrderCount($where, $group = '') {
        return db('pointsorder')->where($where)->group($group)->count();
    }
    ...
    public function getPointOrderAndGoodsList($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '') {
        if (is_array($page)) {
            if ($page[1] > 0) {
                return db('pointsordergoods')->alias('pointsordergoods')->field($field)->join('__POINTSORDER__ pointsorder','pointsordergoods.point_orderid=pointsorder.point_orderid')->where($where)->group($group)->page($page[0], $page[1])->limit($limit)->order($order)->select();
            } else {
                return db('pointsordergoods')->alias('pointsordergoods')->field($field)->join('__POINTSORDER__ pointsorder','pointsordergoods.point_orderid=pointsorder.point_orderid')->where($where)->group($group)->page($page[0])->limit($limit)->order($order)->select();
            }
        } else {
            return db('pointsordergoods')->alias('pointsordergoods')->field($field)->join('__POINTSORDER__ pointsorder','pointsordergoods.point_orderid=pointsorder.point_orderid')->where($where)->group($group)->page($page)->limit($limit)->order($order)->select();
        }
    }
    ...
    public function getPointOrderAndGoodsInfo($where, $field = '*', $order = '', $group = '') {
        return db('pointsordergoods')->alias('pointsordergoods')->field($field)->join('__POINTSORDER__ pointsorder','pointsordergoods.point_orderid=pointsorder.point_orderid')->where($where)->group($group)->order($order)->find();
    }
    ...
    public function createOrder($post_arr, $pointprod_arr, $member_info) {
        ...
        if ($address_info) {
            ...
            $address_array['point_zipcode'] = (isset($address_info['zip_code'])) ? $address_info['zip_code'] : '';
