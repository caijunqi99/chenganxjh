...
class Webchat extends Model
{
    ...
    {
        $result = db('chatmsg')->where($condition)->page($page)->order('m_id')->select();
        return $result;
    }
    ...
    public function addMsg($msg)
    {
        ...
        if ($m_id > 0) {
            $msg['m_id'] = $m_id;
            unset($msg['r_state']);
            db('chatlog')->insert($msg);
            $msg['m_id'] = $m_id;
            $msg['add_time'] = date('Y-m-d H:i:s', $msg['add_time']);
            $t_msg = $msg['t_msg'];
            $goods_id = 0;
            $goods_info = array();
            $pattern = '#' . SHOP_SITE_URL . '/index.php/goods&amp;/index&amp;?goods_id=(\d+)$#';
    ...
    {
        $list = db('chatlog')->where($condition)->order($order)->paginate($page);
    ...
    {
        $list = db('snsfriend')->where($condition)->page($page)->order('friend_addtime desc')->select();
        if (!empty($list) && is_array($list)) {
            foreach ($list as $k => $v) {
                $member = array();
                $u_id = $v['friend_tomid'];
    ...
    public function getGoodsInfo($goods_id)
    {
        ...
        if (is_array($goods) && !empty($goods)) {
            ...
            $goods['goods_promotion_price'] = dsPriceFormat($goods['goods_promotion_price']);
