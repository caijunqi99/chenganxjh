...
class Statisticsgeneral extends BaseSeller
{
    public function _initialize()
    {
        ...
        Lang::load(APP_PATH.'home/lang/zh-cn/statistics.lang.php');
    }
    ...
    public function index(){
        ...
        $statnew_arr['orderamount'] = dsPriceFormat(($t = $stat_order['orderamount'])?$t:(0));
        $statnew_arr['ordermembernum'] = ($t = $stat_order['ordermembernum']) > 0?$t:0;
        $statnew_arr['avgorderamount'] = dsPriceFormat(($t = $stat_order['avgorderamount'])?$t:(0));
        unset($stat_order);
        ...
        $orderlist = $model->statByStatorder($where, $field, 0, 0, 'ordernum desc', '');
        ...
        $statnew_arr['avggoodsprice'] = dsPriceFormat(($t = $stat_ordergoods['avggoodsprice'])?$t:(0));
        unset($stat_ordergoods);

        //商品总数、收藏量
        $goods_list = $model->statByGoods(array('store_id'=>session('store_id'),'is_virtual'=>0),'COUNT(*) as goodsnum, SUM(goods_collect) as gcollectnum');
        $statnew_arr['goodsnum'] = ($t = $goods_list[0]['goodsnum']) > 0?$t:0;
        $statnew_arr['gcollectnum'] = ($t = $goods_list[0]['gcollectnum']) > 0?$t:0;

        //店铺收藏量
        $store_list = $model->getoneByStore(array('store_id'=>session('store_id')),'store_collect');
        ...
        $stat_order = $model->statByStatorder($where, $field, 0, 0, '');
        if($stat_order){
    ...
    public function pricesetting(){
        ...
        if (request()->isPost()){
            ...
            $result = $model->editStoreExtend($update_array,array('store_id'=>session('store_id')));
            if ($result){
        ...
        $pricerange = $model->getfby_store_id(session('store_id'),'pricerange');
    ...
    public function orderprange(){
        ...
        $pricerange = $model->getfby_store_id(session('store_id'),'orderpricerange');
        $pricerange = $pricerange?unserialize($pricerange):array();
        $this->assign('pricerange',$pricerange);
        $this->setSellerCurMenu('Statisticsgeneral');
    ...
    protected function getSellerItemList()
    {
        $menu_array = array(
            array(
                'name' => 'index', 'text' => '店铺概况',
                'url' => url('statisticsgeneral/index')
            ),
             array(
                'name' => 'pricesetting', 'text' => '商品价格区间',
                'url' => url('statisticsgeneral/pricesetting')
            ),  array(
                'name' => 'orderprange', 'text' => '订单金额区间',
