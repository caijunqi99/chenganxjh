...
class Buy_1 extends Model
{
    ...
    public function extendInPay($order_list)
    {
        ...
        if ($order_list[0]['order_type'] == 2) {
            ...
            if ($order_info['order_state'] == ORDER_STATE_PAY) {
                ...
                $update = Model('goods')->editGoods(array(
                                                        'book_buyers' => array(
                                                            'exp', 'book_buyers+1'
                                                        )
                                                    ), array('goods_id' => $order_goods_info['goods_id']));
                if (!$update) {
                    exception('更新商品预定人数失败');
                }
            }
            $condition = array();
    ...
    public function calcCartList($store_cart_list)
    {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            ...
            foreach ($store_cart as $key => $cart_info) {
                $store_cart[$key]['goods_total'] = dsPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
            ...
            $store_goods_total[$store_id] = dsPriceFormat($tmp_amount);
        }
    ...
    public function getMansongRuleCartListByTotal($store_goods_total)
    {
        ...
        foreach ($store_goods_total as $store_id => $goods_total) {
            ...
            if (is_array($rule_info) && !empty($rule_info)) {
                ...
                $rule_info['discount'] = dsPriceFormat($rule_info['discount']);
    ...
    public function getXianshiInfo(& $goods_info, $quantity)
    {
        ...
        if (!config('promotion_allow') || empty($goods_info['xianshi_info']))
            return;
        $goods_info['xianshi_info']['down_price'] = dsPriceFormat($goods_info['goods_price'] - $goods_info['xianshi_info']['xianshi_price']);
    ...
    public function editGoodsNum($buy_items)
    {
        foreach ($buy_items as $goods_id => $buy_num) {
            ...
            if (!$result)
                exception(L('cart_step2_submit_fail'));
        }
    }
    ...
    public function createInvoiceData($invoice_info)
    {
        //发票信息
        $inv = array();
        if (isset($invoice_info['inv_state'])&&$invoice_info['inv_state'] == 1) {
            $inv['类型'] = '普通发票 ';
            $inv['抬头'] = isset($invoice_info['inv_title']) ? $invoice_info['inv_title'] : '个人';
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            foreach ($cart_list[$key]['bl_goods_list'] as $k => $goods_info) {
                if (array_key_exists($goods_info['goods_id'], $goods_online_list)) {
                    ...
                    //每个商品直降多少
                    $total_down_price += $cart_list[$key]['bl_goods_list'][$k]['down_price'] = dsPriceFormat($goods_online_info['goods_price'] - $goods_info['bl_goods_price']);
                }
                else {
                    //商品已经下架
                    $cart_list[$key]['state'] = false;
            ...
            $cart_list[$key]['down_price'] = dsPriceFormat($total_down_price);
        }
    }
