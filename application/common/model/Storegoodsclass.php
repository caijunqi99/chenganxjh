...
class Storegoodsclass extends Model {
    ...
    public function getStoreGoodsClassInfo($param, $field = '*') {
        ...
        return db('storegoodsclass')->where($param)->field($field)->find();
    }
    ...
    public function addStoreGoodsClass($param) {
        ...
        $result = db('storegoodsclass')->insert($param);
        if ($result) {
    ...
    public function editStoreGoodsClass($param, $where) {
        ...
        $result = db('storegoodsclass')->where($where)->update($param);
        if ($result) {
            ...
            foreach ($class_list as $v) {
                if ($v['stc_parent_id'] == 0) {
                    ...
                    if (isset($d[$v['stc_parent_id']]))
                        $d[$v['stc_parent_id']]['child'][] = $v; //防止出现父类不显示时子类被调出
                }
            }
        }
        return $d;
