...
class Groupbuy extends Model
{
    ...
    public function addGroupbuy($param)
    {
        ...
        $param['recommended'] = 0;
        $result = db('groupbuy')->insert($param);
        if ($result) {
            // 更新商品抢购缓存
    ...
    protected function getCachingDataByQuery($key)
    {
        ...
        switch ($key) {
            case 'groupbuy_classes': // 抢购分类
                $classes = db('groupbuyclass')->order('sort asc')->limit(false)->select();
            ...
            case 'groupbuy_vr_classes': // 虚拟抢购分类
                $classes = db('vrgroupbuyclass')->order('class_sort asc')->limit(false)->select();
                foreach ((array)$classes as $v) {
                    $id = $v['class_id'];
            ...
            case 'groupbuy_vr_cities': // 虚拟抢购地区
                // 一级地区 城市
                $arr = (array)db('vrgroupbuyarea')->where(
                    array('hot_city' => 1, 'parent_area_id' => 0,))->order('area_id asc')->limit(false)->select();
                $arr=ds_changeArraykey($arr,'area_id');
                foreach ($arr as $v) {
                    $id = $v['area_id'];
                ...
                if ($pids = array_keys($arr)) {
                    // 二级地区 区域
                    $arr = (array)db('vrgroupbuyarea')->where(
                        array('parent_area_id' => array('in', $pids),
                        ))->order('area_id asc')->limit(false)->select();
                    $arr=ds_changeArraykey($arr,'area_id');
                    foreach ($arr as $v) {
                        $id = $v['area_id'];
                        $pid = $v['parent_area_id'];
                    ...
                    if ($pids = array_keys($arr)) {
                        // 三级地区 街区
                        $arr = (array)db('vrgroupbuyarea')->where(
                            array('parent_area_id' =>
                                array('in', $pids),))->order('area_id asc')->limit(false)->select();
                        $arr=ds_changeArraykey($arr,'area_id');
                        $pids = array_keys($arr);
                        foreach ($arr as $v) {
                            $id = $v['area_id'];
