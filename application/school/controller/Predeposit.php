...
class Predeposit extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'school/lang/zh-cn/predeposit.lang.php');
    }
    ...
    public function recharge_edit() {
        ...
        if ($id <= 0) {
            $this->error(lang('admin_predeposit_parameter_error'), 'School/Predeposit/pdrecharge_list');
        }
        //查询充值信息
        $model_pd = Model('predeposit');
        $condition = array();
        ...
        if (empty($info)) {
            $this->error(lang('admin_predeposit_record_error'), 'School/Predeposit/pdrecharge_list');
        }
        if (!request()->isPost()) {
            //显示支付接口列表
...
//            $model_pd->startTrans();
            ...
            $model_pd->commit();
            $this->log($log_msg, 1);
            $this->success(lang('admin_predeposit_recharge_edit_success'), 'School/Predeposit/pdrecharge_list');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg, 0);
            $this->error($e->getMessage(), 'School/Predeposit/pdrecharge_list');
        }
    }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                ...
                'text' => '充值明细',
                'url' => url('School/Predeposit/pdrecharge_list')
            ),
            array(
                'name' => 'pdcash_list',
                'text' => '提现管理',
                'url' => url('School/Predeposit/pdcash_list')
            ),
            array(
                'name' => 'pdlog_list',
                'text' => '预存款明细',
                'url' => url('School/Predeposit/pdlog_list')
            ),
            array(
                'name' => 'pd_add',
                'text' => '预存款调节',
                'url' => url('School/Predeposit/pd_add')
            ),
        );
        return $menu_array;
    }
